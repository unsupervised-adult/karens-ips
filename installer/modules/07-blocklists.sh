...
add_dataset_rules_to_config() {
    ...
    # Create empty SLIPS integration rules file (will be populated by sync service)
    local slips_rules_file="/var/lib/suricata/rules/slips-integration.rules"
    if [[ ! -f "$slips_rules_file" ]]; then
        cat > "$slips_rules_file" << 'EOF'
# SLIPS ↔ Suricata Integration Rules
# Auto-generated by slips-suricata-sync service
# Do not edit manually - changes will be overwritten

# Placeholder - rules will be generated when SLIPS detects threats
EOF
        log "Created empty SLIPS integration rules file"
    fi
